...
func (p *ParticipantRepo) CreateParticipant(participant models.Participant) (models.Participant, error) {
	...
	err := p.db.QueryRow(query, participant.FirstName, participant.LastName, participant.Email, participant.BirthDate, participant.Username, participant.Password, participant.Role).Scan(&participant.ID)
	if err != nil {
...
func (p *ParticipantRepo) GetParticipant(id int) (models.Participant, error) {
	...
	err := p.db.QueryRow(query, id).Scan(&participant.ID, &participant.FirstName, &participant.LastName, &participant.Email, &participant.BirthDate, &participant.Username, &participant.Password, &participant.Role)
...
func (p *ParticipantRepo) UpdateParticipant(participant models.Participant) (models.Participant, error) {
	...
	err := p.db.QueryRow(query, participant.FirstName, participant.LastName, participant.Email, participant.BirthDate, participant.Username, participant.Password, participant.Role, participant.ID).Scan(&participant.ID)
	if err != nil {
		return models.Participant{}, err
	}
	return participant, nil
